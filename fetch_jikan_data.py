...
def search_jikan(anime_title):
    """Search Jikan API (MyAnimeList) for anime"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('data') and len(data['data']) > 0:
                ...
                if mal_id:
                    mal_url = anime.get('url', f"https://myanimelist.net/anime/{mal_id}")
                    ...
                    print(f"  ⚠️  No MAL ID found for: {anime_title}")
                    return None
            else:
                print(f"  ⚠️  No results found for: {anime_title}")
                return None
        elif response.status_code == 429:
            print(f"  ⚠️  Rate limited, waiting 60 seconds...")
            time.sleep(60)
            return search_jikan(anime_title)  # Retry
        else:
            print(f"  ❌ API error (status {response.status_code}): {anime_title}")
            return None

    except Exception as e:
        print(f"  ❌ Error searching {anime_title}: {str(e)}")
        return None

def update_anime_json():
    """Update anime.json with data from Jikan API"""
    ...
    print(f"Processing {total} anime...\n")
    ...
    for i, item in enumerate(items, 1):
        ...
        anime_data = search_jikan(title)

        if anime_data:
            item['link'] = anime_data['link']
            item['cover'] = anime_data['cover']
            updated += 1
    ...
    print(f"\n✅ Done! Updated {updated} anime, skipped {skipped} anime")
